...
(function (global, factory) {
  ...
  // #9446: Firefox <= 53 (in particular, esr 52) has incorrect event.timestamp implementation and does not fire microtasks in between event propagation, so safe to exclude. var usemicrotaskfix="isUsingMicroTask" && !(isff number(isff[1]) <="53);" function add$1 ( name, handler, capture, passive ) { async edge case #6566: inner click triggers patch, handler attached outer element during triggered again. this happens because browsers microtask ticks propagation. the solution is simple: we save timestamp when a attached, would only if passed it was fired after attached. (usemicrotaskfix) attachedtimestamp="currentFlushTimestamp;" original="handler;" = (e) no bubbling, should always fire. just safety net unreliable certain weird environments... e.target="==" e.currenttarget || attachment e.timestamp>= attachedTimestamp ||
          ...
          e.timeStamp <= 0 || #9448 bail if event is fired in another document a multi-page electron nw.js app, since event.timestamp will be using different starting reference e.target.ownerdocument !="=" ) { return original.apply(this, arguments) } }; target$1.addeventlistener( name, handler, supportspassive ? capture: capture, passive: passive : capture ); function remove$2 ( _target (_target target$1).removeeventlistener( handler._wrapper updatedomlisteners (oldvnode, vnode) (isundef(oldvnode.data.on) && isundef(vnode.data.on)) var on="vnode.data.on" {}; oldon="oldVnode.data.on" target$1="vnode.elm;" normalizeevents(on); updatelisteners(on, oldon, add$1, remove$2, createoncehandler$1, vnode.context); events="{" create: updatedomlisteners, update: * svgcontainer; updatedomprops (isundef(oldvnode.data.domprops) isundef(vnode.data.domprops)) key, cur; elm="vnode.elm;" oldprops="oldVnode.data.domProps" props="vnode.data.domProps" clone observed objects, as the user probably wants to mutate it (isdef(props.__ob__)) = extend({}, props); for (key oldprops) (!(key props)) elm[key] ; props) cur="props[key];" ignore children node has textcontent or innerhtml, these throw away existing dom nodes and cause removal errors subsequent patches (#3360) 'textcontent' key="==" 'innerhtml') (vnode.children) vnode.children.length="0;" (cur="==" oldprops[key]) continue #6601 work around chrome version <="55" bug where single textnode replaced by innerhtml retains its parentnode property (elm.childnodes.length="==" 1) elm.removechild(elm.childnodes[0]); 'value' elm.tagname 'progress') store value _value well non-string values stringified elm._value="cur;" avoid resetting cursor position when same strcur="isUndef(cur)" '' string(cur); (shouldupdatevalue(elm, strcur)) elm.value="strCur;" else 'innerhtml' issvg(elm.tagname) isundef(elm.innerhtml)) ie doesn't support svg elements svgcontainer="svgContainer" document.createelement('div'); svgcontainer.innerhtml="<svg>" + "<>";
